"use client";
import { homePageData, getUserInfo } from "@/services/dataAPI";
...
const Home = () => {
  ...
  // Get user's first name from email or username
  const getUserDisplayName = () => {
    if (!user) return '';
    
    // Try to get name from username first
    if (user.userName) {
      return user.userName.split(' ')[0]; // Get first name only
    }
  ...
  const greetingText = displayName ? `${salutation}, ${displayName}` : salutation;

  return (
    <div>
      <OnlineStatus />
      <h1 className='text-4xl font-bold mx-2 m-9 text-white flex gap-2'>"{greetingText}  <GiMusicalNotes />"</h1>
